...
class FileSaveHelper(private val mContentResolver: ContentResolver) : LifecycleObserver {
    ...
    private val observer = Observer { fileMeta: FileMeta -> run {
        if(resultListener != null) {
            resultListener!!.onFileCreatedResult(
                fileMeta.isCreated,
                fileMeta.filePath,
                fileMeta.error,
                fileMeta.uri
    ...
    fun createFile(fileNameToSave: String, listener: OnFileCreateResult) {
        ...
        executor!!.submit {
            ...
            try {
                ...
                val editedImageUri: Uri = getEditedImageUri(fileNameToSave, newImageDetails, imageCollection)
    ...
    interface OnFileCreateResult {
         ...
         */
        fun onFileCreatedResult(created: Boolean, filePath: String?, error: String?, uri: Uri?)
    }
